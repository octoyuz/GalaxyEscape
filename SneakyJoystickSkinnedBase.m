...
- (void) setContentSize:(CGSize)s
{
	...
	joystick.joystickRadius = s.width/4;
}
...
- (void) setThumbSprite:(CCSprite *)aSprite
{
	...
	if(aSprite){
		...
		[joystick setThumbRadius:thumbSprite.contentSize.width/4];
	}
}
...
- (void) setJoystick:(SneakyJoystick *)aJoystick
{
	...
	if(aJoystick){
		...
		if(backgroundSprite)
			[joystick setJoystickRadius:backgroundSprite.contentSize.width/4];
	}
}
